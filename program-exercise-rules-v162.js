...
(()=>{
  ...
  window.programExerciseForm=function(x){
    ...
    const tempo=e?.tempo||recommendedTempo(kind),tempoLight=e?.tempoLight||second.tempo||'3-1-2';
    ...
    modal(`<div class="px-exercise-head"><button class="px-icon-btn" onclick="openProgramEditor('${x.pid}',${x.wi},${x.di})">←</button><div class="px-head-copy grow"><h2>${esc(x.n)}</h2><div class="muted">Настройка упражнения</div></div></div>
      <input id="pmKind" type="hidden" value="${kind}"><input id="pmMethod" type="hidden" value="${method}"><input id="pmRestMode" type="hidden" value="${restMode}">
      ...
      <div class="px-main-grid">
        ...
        <div class="field"><label>Вес, кг</label><input id="pmWeight" inputmode="decimal" value="${first.w||0}"></div>
        <div class="field"><label>Целевой RPE</label><input id="pmRpe" inputmode="decimal" value="${e?.rpe||8}"></div>
  ...
  window.saveProgramExercise=function(pid,wi,di,nameToken,sourceToken,bpToken,tgToken,eqToken,existingIndex){
    ...
    const data={tempo,tempoLight,heavyReps,lightReps,lightWeight,middleSets,middleReps,middleWeight,sldrSets,sldrDrop},sets=buildSets(method,count,w,r,rest,data),obj={...(old||{}),id:old?.id||uid('pex'),n,sourceId:decodeURIComponent(sourceToken||'')||null,bp:decodeURIComponent(bpToken||''),tg:decodeURIComponent(tgToken||''),eq:decodeURIComponent(eqToken||''),kind,method,rpe,tempo,tempoLight:method==='UNVRSL'?tempoLight:null,restMode,rest,innerRest:INNER_REST[method],heavyReps:method==='UNVRSL'?heavyReps:null,lightReps:method==='UNVRSL'?lightReps:null,lightWeight:method==='UNVRSL'?lightWeight:null,middleSets:method==='UNVRSL'?middleSets:null,middleReps:method==='UNVRSL'?middleReps:null,middleWeight:method==='UNVRSL'?middleWeight:null,miniSets:method==='SLDR'?sldrSets:null,repDrop:method==='SLDR'?sldrDrop:null,note,sets};
