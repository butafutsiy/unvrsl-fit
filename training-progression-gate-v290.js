'use strict';
(()=>{
  const W=window,D=document,REV=290;
  if(W.__unvrslTrainingProgressionGateV290)return;
  ...
  const SPECIAL=/UNVRSL|SLDR|\bDS\b|DROP|FST-?7/i;
  ...
  function migrateActualFields(){
    ...
    const hist=Array.isArray(W.st?.sessions)?W.st.sessions.slice(-12):[];all.push(...hist);
  ...
  async function cloudLatest(ex,cur,currentRange){
    try{
      ...
      if(q.error)return[];
      for(const row of q.data||[]){const p=row.payload||{};if(String(p.id||'')===String(cur?.id||''))continue;const rows=rowsFromSession({...p,date:p.date||row.workout_date,workout_date:row.workout_date},ex,currentRange,cur);if(rows.length)return rows}
    }catch(e){console.warn('UNVRSL progression gate cloud history',e)}
    return[]
  }
  ...
  function evaluateGate(ex,cur,rows){
    ...
    const topNeed=Math.max(1,Math.ceil(normalized.length*.75));
    const topReady=topCount>=topNeed;
    const rpeDelta=avgRpe-target;
    const hasActualEffort=actualRpes.length>0;
  ...
  function isDeload(cur){
    const max=N(cur?.programWeekIntensityMax);
    if(max>0&&max<=67)return true;
    if(!(cur?.programId||cur?.planId||cur?.programName)&&[4,6].includes(Number(cur?.w)))return true;
    return false
  }
  ...
  function gateWeight(modelRec,prev,gate,ex,cur,stp){
    let rec=N(modelRec);if(!(rec>=0)||!(prev>=0)||!gate)return rec;
    if(isDeload(cur))return rec;
    const isAssist=assisted(ex);
    if(gate.status==='HOLD'){
      if(harder(rec,prev,isAssist))return prev;
      return rec
    }
  ...
  async function applyGroup(group,cur){
    ...
    group.forEach(ex=>{
      ex.trainingProgression290={...gate,previousWeight:prev,step:stp,deload:isDeload(cur)};
      (ex.set||[]).forEach(set=>{
        if(set.ok||set.manualOverride)return;
        const before=N(set.recommendedW);
        if(before==null)return;
        const after=round(gateWeight(before,prev,gate,ex,cur,stp),stp);
        set.progressionGateV290={...gate,modelRecommendedW:before,finalRecommendedW:after};
        if(after!=null&&Math.abs(after-before)>.001){set.recommendedW=after;changed=true}
        if(ex.programWeightMode!=='prescribed'&&after!=null){
          if(N(set.plannedW)!==after){set.plannedW=after;set.baselineW=after;set.baselineSource='adaptive_load_model_v260+rep_gate_v290';changed=true}
  ...
  function exerciseGroups(cur){
    const map=new Map();
    (cur?.ex||[]).forEach(ex=>{const k=key(ex);if(!map.has(k))map.set(k,[]);map.get(k).push(ex)});
    return[...map.values()]
  }
  async function applyGates(cur){
  ...
  function fixBodyweightReadiness(cur){
    let changed=false;
    (cur?.ex||[]).forEach(ex=>{
      if(exerciseClass(ex)!=='bodyweight')return;
      const stp=equipmentStep(ex);
      (ex.set||[]).forEach(set=>{
        if(set.ok||set.manualOverride||!(N(set.plannedW)>=0))return;
        const today=readinessWeight(set.plannedW,ex,cur,stp);
        if(today!=null&&N(set.w)!==today){set.w=today;changed=true}
      })
    });
    if(changed)try{W.save?.()}catch(_){}
    return changed
  }
  ...
  function refreshDom(cur){
    const root=D.getElementById('start');if(!root)return;
    ...
    (cur.ex||[]).forEach((ex,i)=>{
      ...
      const group=(cur.ex||[]).filter(x=>key(x)===k),meta=group.map(x=>x.trainingProgression290).find(Boolean);if(!meta)return;
      const card=cards[i];if(!card)return;
      const rp=readinessPercent(cur);
      const rec=card.querySelector('.te200-rec');
      if(rec){
        ...
        const b=rec.querySelector('b'),sp=rec.querySelector('span');if(b&&today.length)b.textContent=`Рекомендация · ${fmtWeights(today)} кг`;
        if(sp)sp.textContent=`${meta.reason}${rp?` · самочувствие ${rp}`:''}`;
      }
      const auto=card.querySelector('.te200-auto');
      if(auto){
        const today=[];group.forEach(g=>(g.set||[]).forEach(s=>{if(N(s.w)!=null)today.push(N(s.w))}));
        if(today.length)auto.textContent=`Автовес · ${fmtWeights(today)} кг · ${meta.reason}${rp?` · самочувствие ${rp}`:''}`;
      }
    })
  }

  let busy=false,lastSig='';
  function signature(cur){return JSON.stringify([cur?.id||'',cur?.trainingLoadModelAt||'',cur?.trainingPreparedAt||'',cur?.trainingReadinessDone?N(cur?.readiness?.factor)||1:1,(W.st?.sessions||[]).length,(cur?.ex||[]).map(ex=>[key(ex),ex.programWeightMode,(ex.set||[]).map(s=>[N(s.targetRepMin),N(s.targetRepMax),N(s.targetRpeResolved),N(s.recommendedW)])])])}
  async function sync(force=false){
    const cur=W.st?.current;if(!cur?.id||cur.ended)return;
    fixBodyweightReadiness(cur);
    const sig=signature(cur);
    if(!busy&&(force||sig!==lastSig)&&cur.trainingLoadModelAt){
      busy=true;try{await applyGates(cur);lastSig=signature(cur);try{W.startPage?.();W.trainingEngine200Tick?.()}catch(_){}}catch(e){console.warn('UNVRSL progression gate v290',e)}finally{busy=false}
    }
    refreshDom(cur)
  }
  ...
  [250,700,1400,2600,4500].forEach(ms=>setTimeout(()=>sync(ms===1400),ms));
  setInterval(()=>sync(false),700);
})();