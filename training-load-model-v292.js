'use strict';
(()=>{
  const W=window,D=document,REV=292,CONTROL_REV=315,LAMBDA=.65;
  ...
  async function calculateGroup(group,cur){
    ...
    const flat=group.items.flatMap(({ex,i})=>(ex.set||[]).map((set,si)=>({ex,set,ei:i,si}))),ranges=flat.map(x=>targetRange(x.ex,x.set)).filter(Boolean),futureRange=ranges.length?{lo:mean(ranges.map(x=>x.lo)),hi:mean(ranges.map(x=>x.hi))}:targetRange(first),stp=equipmentStep(first,est.rows),m=method(first),programWeights=flat.map(x=>num(x.set.programW));let values=null,gate=evaluateGate(first,cur,hist,futureRange),prev=previousWeight(est);
    ...
    group.items.forEach(({ex})=>{if(ex.programWeightMode!=='prescribed')ex.weightDecision=values.some(v=>v!=null)?'adaptive_auto':'calibration';if(gate){ex.trainingProgression292={...gate,previousWeight:prev,step:stp,deload:isDeload(cur)};ex.trainingProgression315=ex.trainingProgression292;ex.trainingProgression291=ex.trainingProgression292;ex.trainingProgression290=ex.trainingProgression292}ex.trainingEstimate200={...(ex.trainingEstimate200||{}),e1rm:+est.e1rm.toFixed(1),previousWeight:prev,...confidenceMeta(ex,cur,est)}});return true
  }
