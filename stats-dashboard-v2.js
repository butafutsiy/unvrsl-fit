...
(()=>{
  ...
  function dashboardHtml(){
    const ws=dataWorkouts(),weights=dataWeights(),delta=weight30(weights),deltaText=delta==null?'—':`${delta>0?'+':''}${fmt(delta)} кг`;
    return `<div class="sd2-head"><div><h1>Статистика</h1><div class="sd2-sub">Прогресс и история</div></div><button class="sd2-history" onclick="nav('history')" aria-label="История">↶</button></div><div class="sd2-grid">${metric('Тренировки',ws.length,'◫')}${metric('Этот месяц',monthCount(ws),'▣')}${metric('Серия недель',weekStreak(ws),'♨')}${metric('Вес 30 дн.',deltaText,'▢')}</div>${heatmapHtml(ws)}${weightCardHtml(weights)}${measuresHtml()}<div class="sd2-section">СИЛОВЫЕ</div><div class="sd2-strength-host">${typeof window.profileStrengthOverviewHtml==='function'?window.profileStrengthOverviewHtml():typeof profileStrengthOverviewHtml==='function'?profileStrengthOverviewHtml():'<div class="sd2-card sd2-empty">Силовой прогресс появится после тренировок.</div>'}</div>`
  }

  function renderDashboard(){const root=document.getElementById('stats');if(!root)return;root.classList.add('stats-v2');root.innerHTML=dashboardHtml();requestAnimationFrame(()=>{const h=document.getElementById('sd2HeatWrap');if(h)h.scrollLeft=h.scrollWidth})}
