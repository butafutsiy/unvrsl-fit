...
(()=>{
  ...
  function dashboardHtml(){
    ...
    return `<div class="sd2-head"><div><h1>Статистика</h1><div class="sd2-sub">Прогресс и история</div></div></div><div class="sd2-grid">${metric('Тренировки',ws.length,'◫')}${metric('Этот месяц',monthCount(ws),'▣')}${metric('Серия недель',weekStreak(ws),'♨')}${metric('Средний RPE',avgRpe,'◉')}</div>${heatmapHtml(ws)}${measuresHtml()}<div class="sd2-section">СИЛОВЫЕ</div><div class="sd2-strength-host">${typeof window.profileStrengthOverviewHtml==='function'?window.profileStrengthOverviewHtml():typeof profileStrengthOverviewHtml==='function'?profileStrengthOverviewHtml():'<div class="sd2-card sd2-empty">Силовой прогресс появится после тренировок.</div>'}</div>${workoutHistoryHtml()}`
  }

  function renderDashboard(){const root=document.getElementById('stats');if(!root)return;const html=dashboardHtml();root.classList.add('stats-v2');root.dataset.statsAuthority='252';if(root.__statsDashboardHtml===html&&root.querySelector('.sd2-head'))return;const y=root.classList.contains('active')?(window.scrollY||document.documentElement?.scrollTop||0):0;root.innerHTML=html;root.__statsDashboardHtml=html;if(y>0)window.scrollTo({top:y,left:0,behavior:'auto'});requestAnimationFrame(()=>{const h=document.getElementById('sd2HeatWrap');if(h)h.scrollLeft=h.scrollWidth})}
