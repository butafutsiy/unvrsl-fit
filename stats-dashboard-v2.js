...
(()=>{
  ...
  function renderDashboard(){const root=document.getElementById('stats');if(!root)return;const html=dashboardHtml();root.classList.add('stats-v2');if(root.__statsDashboardHtml===html)return;const y=root.classList.contains('active')?(window.scrollY||document.documentElement?.scrollTop||0):0;root.innerHTML=html;root.__statsDashboardHtml=html;if(y>0)window.scrollTo({top:y,left:0,behavior:'auto'});requestAnimationFrame(()=>{const h=document.getElementById('sd2HeatWrap');if(h)h.scrollLeft=h.scrollWidth})}
