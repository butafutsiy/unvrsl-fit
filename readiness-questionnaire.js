...
(()=>{
  ...
  const state={values:{sleep:null,energy:null,stress:null,recovery:null},manual:null,lastAuto:0,pendingProgram:null};
  ...
  style.textContent=`
    ...
    .rq227-scale-wrap{min-width:0}.rq227-scale{display:grid;grid-template-columns:repeat(5,1fr);gap:7px}.rq227-score{height:48px;border-radius:15px;background:#2a2a2e;border:1px solid transparent;color:#a9a9af;font-weight:800;font-size:17px}.rq227-score.on{background:var(--green);color:#061108;border-color:transparent}.rq227-score:active{transform:scale(.97)}.rq227-scale-ends{display:flex;justify-content:space-between;gap:10px;margin:4px 2px 0;color:#707075;font-size:10px;line-height:1.1}
    ...
    @media(max-width:520px){.rq227{padding:0 0 2px}.rq227 .sheet-grabber{margin-bottom:8px}.rq227 h2{font-size:26px!important;margin:0 0 5px!important}.rq227-sub{font-size:14px;margin-bottom:13px}.rq227-rows{gap:10px}.rq227-row{grid-template-columns:110px minmax(0,1fr);gap:7px}.rq227-label{font-size:13.5px;letter-spacing:-.35px;white-space:nowrap}.rq227-scale{gap:5px}.rq227-score{height:41px;border-radius:13px;font-size:15px}.rq227-scale-ends{font-size:9px;margin-top:3px}.rq227-result{margin:13px 0 11px;padding:12px 13px;border-radius:17px;gap:8px}.rq227-result b{font-size:15px}.rq227-result span{font-size:12px;margin-top:3px}.rq227-badge{min-width:52px;padding:8px 9px;font-size:14px}.rq227-manual-title{font-size:12px;margin:2px 2px 7px}.rq227-manual{grid-template-columns:repeat(5,1fr);gap:6px}.rq227-manual button{min-height:39px;border-radius:12px;font-size:12px;padding:6px 2px}.rq227-actions{margin-top:12px}.rq227-start{min-height:47px;font-size:16px!important;border-radius:15px!important}.rq227-plan{min-height:45px;margin-top:7px!important;font-size:15px!important;border-radius:15px!important}.rq227-note{font-size:9px;margin-top:7px}}
  `;
  ...
  function rememberProgramStart(target){
    ...
    const m=src.match(/(?:beginProgramDay|programStartFromEditorV382|programStartFromEditorV386)\([^'\"]*['\"]([^'\"]+)['\"]\s*,\s*(\d+)\s*,\s*(\d+)\)/);
    if(!m)return;
  ...
  function pendingProgramSubtitle(){
    ...
    return d?`W${x.wi+1} · ${d.name||`День ${x.di+1}`}`:null
  }
  function workoutSubtitle(){
    const pending=pendingProgramSubtitle();
    if(pending)return pending;
    const cur=W.st?.current;
    if(cur?.c)return `${cur.w?`W${cur.w} · `:''}${cur.c}`;
  ...
  function scoreAndPercent(){
    ...
    const pos=x=>(x-1)/4,neg=x=>(5-x)/4;
    const score=Math.round(pos(v.sleep)*25+pos(v.energy)*30+pos(v.recovery)*30+neg(v.stress)*15);
    let percent=0;if(score<30)percent=-10;else if(score<50)percent=-7.5;else if(score<70)percent=-5;else if(score<85)percent=-2.5;
    state.lastAuto=percent;return{ready:true,score,percent}
  }
  function hiddenInputs(){
    const v=state.values,t=x=>x==null?'0':'1';
    return `<input type="hidden" id="te200Sleep" data-touched="${t(v.sleep)}" value="${v.sleep??3}"><input type="hidden" id="te200Energy" data-touched="${t(v.energy)}" value="${v.energy??3}"><input type="hidden" id="te200Sore" data-touched="${t(v.recovery)}" value="${v.recovery==null?3:6-v.recovery}"><input type="hidden" id="te200Stress" data-touched="${t(v.stress)}" value="${v.stress??3}">`
  }
  function metricRow(key,label,left,right){
    const selected=state.values[key];
    return `<div class="rq227-row"><div class="rq227-label">${label}</div><div class="rq227-scale-wrap"><div class="rq227-scale">${[1,2,3,4,5].map(n=>`<button type="button" class="rq227-score${selected===n?' on':''}" onclick="readinessUiSetV227('${key}',${n})">${n}</button>`).join('')}</div><div class="rq227-scale-ends"><span>${left}</span><span>${right}</span></div></div></div>`
  }
  function manualButtons(){return[-10,-7.5,-5,-2.5,0,2.5,5,7.5,10].map(p=>`<button type="button" class="${state.manual===p?'on':''}" onclick="readinessUiManualV227(${p})">${fmt(p)}</button>`).join('')}
  function markup(){
  ...
  function applyManualWhenReady(percent,before){let tries=0;const timer=setInterval(()=>{tries++;const cur=W.st?.current,newSession=cur&&cur!==before&&cur?.id,existingSession=cur&&cur===before&&cur?.id,prepared=cur?.trainingReadinessDone&&(cur?.trainingEngineRevision||tries>18);if((newSession||existingSession)&&prepared){clearInterval(timer);setTimeout(()=>applyManualToCurrent(percent,cur),80)}else if(tries>50)clearInterval(timer)},80)}

  function revealWorkout(reason){
    D.documentElement.classList.remove('te200-preparing');
    const cur=W.st?.current;
    if(cur?.id&&!cur?.ended){
      try{W.closeModal?.()}catch(_){ }
      try{if(typeof W.nav==='function')W.nav('start');else W.startPage?.()}catch(_){try{W.startPage?.()}catch(__){ }}
    }
    if(reason)console.warn('UNVRSL readiness start recovery',reason)
  }
  function confirmSafely(adjust){
    const original=W.trainingConfirmReadiness200;
    if(typeof original!=='function'){W.toast?.('Модуль тренировки ещё загружается');return}
    let out;
    try{out=original(adjust)}catch(error){revealWorkout(error);W.toast?.('Тренировка открыта, расчёт продолжится автоматически');state.pendingProgram=null;return}
    Promise.resolve(out).catch(error=>{revealWorkout(error);W.toast?.('Тренировка открыта, расчёт продолжится автоматически')}).finally(()=>{state.pendingProgram=null});
    setTimeout(()=>{if(D.documentElement.classList.contains('te200-preparing'))revealWorkout('prepare watchdog')},4000)
  }

  W.readinessUiStartV227=usePlan=>{
    const r=scoreAndPercent();
    if(!usePlan&&!r.ready){W.toast?.('Оцени все четыре показателя');return}
    if(usePlan){confirmSafely(false);return}
    if(state.manual==null){confirmSafely(true);return}
    const before=W.st?.current||null,percent=state.manual;applyManualWhenReady(percent,before);confirmSafely(false)
  };
