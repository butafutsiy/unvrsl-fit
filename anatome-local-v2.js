...
(()=>{
  ...
  style.textContent=`
    ...
    #anatomeMuscleCard .anatome-figure{min-height:440px!important;padding:0!important;background:radial-gradient(circle at 50% 42%,rgba(10,132,255,.08),rgba(191,90,242,.05) 43%,transparent 72%)!important}
    ...
    #anatomeMuscleCard .anatome-muscle[data-load-level="3"] .anatome-bar i{background:#ff375f!important}
    #anatomeMuscleCard .anatome-muscle[data-load-level="2"] .anatome-bar i{background:#bf5af2!important}
    #anatomeMuscleCard .anatome-muscle[data-load-level="1"] .anatome-bar i{background:#0a84ff!important}
  ...
  function weekTonnage(){
    const cut=new Date();cut.setHours(0,0,0,0);cut.setDate(cut.getDate()-6);let total=0;
    (st?.sessions||[]).forEach(s=>{const d=new Date(String(s?.date||'')+'T12:00:00');if(Number.isNaN(d.getTime())||d<cut)return;(s.ex||[]).forEach(e=>(e.set||[]).forEach(x=>{if(!x?.ok)return;const w=Number(x.w),r=Number(x.r);if(Number.isFinite(w)&&Number.isFinite(r)&&w>0&&r>0)total+=w*r}))});
  ...
  function scoresFromCard(card){
    ...
    rows.sort((a,b)=>b[1]-a[1]);const max=rows[0]?.[1]||1;
    rows.forEach(([,v,el])=>{const q=v/max;el.dataset.loadLevel=q>=.67?'3':q>=.34?'2':'1'});
    return new Map(rows.map(([s,v])=>[s,v]));
  }

  function colorFor(slug,scores){
    const v=scores.get(slug)||0;if(!v)return'#34343a';
    const max=Math.max(1,...scores.values()),q=v/max;
    return q>=.67?'#ff375f':q>=.34?'#bf5af2':'#0a84ff';
  }
  ...
  function sideSvg(side,scores){
    ...
    return `<svg class="anatome-local-side" viewBox="40 140 640 1230" preserveAspectRatio="xMidYMid meet" aria-label="${side==='front'?'Мышцы спереди':'Мышцы сзади'}">${paths.join('')}</svg>`;
  }

  async function loadBody(){
    if(bodyData)return bodyData;
    try{const r=await fetch(BODY_URL,{cache:'default'});if(!r.ok)throw new Error(`HTTP ${r.status}`);bodyData=await r.json();window.UNVRSL_ANATOME_BODY_PATHS=bodyData;return bodyData}catch(e){console.warn('local Anatome body',e);return null}
  }
  ...
  async function upgradeCard(){
    ...
    let ton=card.querySelector('.anatome-tonnage-local');
    if(!ton){ton=document.createElement('div');ton.className='anatome-tonnage-local';ton.innerHTML='<span>Недельный тоннаж</span><b></b>';const body=card.querySelector('.anatome-body');body?.before(ton)}
    const tb=ton.querySelector('b');if(tb)tb.textContent=`${weekTonnage().toLocaleString('ru-RU')} кг`;
    const scores=scoresFromCard(card);if(!scores.size)return;
    if(!await loadBody())return;
    const fig=card.querySelector('.anatome-figure');if(!fig)return;
    const sig=[...scores.entries()].map(x=>x.join(':')).join('|');if(fig.dataset.localSig===sig)return;fig.dataset.localSig=sig;
  ...
  function watch(){
    ...
    new MutationObserver(run).observe(root,{childList:true,subtree:true,characterData:true});run();
  }
