'use strict';
(()=>{
  if(window.__unvrslLegacyRetirementV254)return;
  window.__unvrslLegacyRetirementV254=true;

  const retired=[
    'legacy-retirement-v253.js','og-style-legacy-v157.js',
  ...
  window.UNVRSL_RETIRED_SCRIPTS_V254=Object.freeze(retired.slice());
  ...
  style.id='legacy-retirement-v254-style';
  style.textContent=`
  ...
  const oldGlobals=['anatomeMuscleCardHtmlV253','anatomeMountCardV253','unvrslStatsSessions208','statsOpenWorkout208','statsWeightRange','statsWeightSheet','statsSaveWeight','statsGoalSheet','statsSaveGoal','statsEnsureCanonicalV253'];
  let queued=false;
  ...
  function install(){
    for(const id of ['home','stats','sheet']){
      const node=document.getElementById(id);if(!node||node.__legacyRetirementV254Observer)continue;
      const observer=new MutationObserver(schedule);observer.observe(node,{childList:true,subtree:true});node.__legacyRetirementV254Observer=observer;
    }
    schedule();
  }
