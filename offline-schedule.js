...
(()=>{
  ...
  if(W.__unvrslOfflineScheduleV391)return;
  W.__unvrslOfflineScheduleV391=true;W.__unvrslOfflineScheduleV390=true;
  ...
  async function renderOffline(){
    ...
    const token=++cache.renderToken;if(!cache.loaded||!root.children.length)root.innerHTML='<div class="card muted">Загружаю расписание…</div>';
    try{await loadData();if(token!==cache.renderToken)return;
      root.innerHTML=`${scheduleHtml()}${metricsHtml()}<div class="section">ОФЛАЙН-КЛИЕНТЫ</div><div id="offlineClientList">${cache.clients.length?cache.clients.map(clientCard).join(''):'<div class="card"><div class="title">Пока нет офлайн-клиентов</div><div class="muted" style="margin-top:6px">Добавь клиента, абонемент и его постоянное расписание.</div></div>'}</div>`
  ...
  W.offlineSelectWeekdayV390=function(day){cache.selectedDay=Math.max(1,Math.min(7,Number(day)||1));const root=D.getElementById('offlineClientsPane');if(root&&cache.loaded)root.innerHTML=`${scheduleHtml()}${metricsHtml()}<div class="section">ОФЛАЙН-КЛИЕНТЫ</div><div id="offlineClientList">${cache.clients.length?cache.clients.map(clientCard).join(''):'<div class="card muted">Клиентов пока нет.</div>'}</div>`};
  ...
  function install(){
    ...
    if(typeof detail==='function'&&!detail.__scheduleV390){const wrapped=async function(id){const result=await detail.apply(this,arguments);enhanceClientDetail(id);return result};wrapped.__scheduleV390=true;W.offlineClientDetail=wrapped;try{offlineClientDetail=wrapped}catch(_){ }}
  ...
  W.unvrslOfflineScheduleV391=W.unvrslOfflineScheduleV390={DAYS,weekday,monday,dateForWeekday,scheduleSummary,occurrenceRows,finance,hydrate:value=>Object.assign(cache,value||{}),version:REV,render:renderOffline};
  install();
})();
