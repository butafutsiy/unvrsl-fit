'use strict';
(()=>{
  const W=window,D=document,RELEASE=388,READY_CLASS='unvrsl-shell-ready-v316',LEGACY_READY_CLASS='unvrsl-app-ready-v260';
  if(W.__unvrslStartupOrchestratorV388)return;
  W.__unvrslStartupOrchestratorV388=true;
  ...
  // v388 root fix: the base shell must not wait for decorative/runtime patches.
  // Disable the old watchdog workaround. Startup now completes only when the
  // actual base app + ui-stability layer are ready.
  try{
  ...
  function setProgress(value){
    ...
    const track=splash?.querySelector('.u-progress');
    const bar=track?.querySelector('i');
    const label=splash?.querySelector('.u-status');
    progressValue=next;
    if(bar&&bar.style.width!==`${next}%`)bar.style.width=`${next}%`;
  ...
  W.unvrslStartupAssetLoadedV320=()=>{
    loadedAssets+=1;
    setProgress(Math.min(78,24+loadedAssets*1.1));
  };
  W.unvrslStartupAssetLoadedV319=W.unvrslStartupAssetLoadedV320;
  setProgress(progressValue);

  // Optional feature modules are intentionally outside the startup barrier.
  function loadTrainingLoadModel(){
    ...
    s.src='training-load-model.js?v=387';
    s.async=false;
    s.dataset.unvrslLoadModelV292='1';
    s.onerror=()=>console.warn('UNVRSL load model v292 failed to load');
  ...
  function loadProgramIntensity(){
    ...
    const s=D.createElement('script');
    s.src='program-intensity-autoweight.js?v=387';
    s.async=false;
    s.dataset.unvrslProgramIntensityV261='1';
    s.onerror=()=>console.warn('UNVRSL program intensity UI failed to load');
    D.body?.appendChild(s);
  }
  function loadTrainerClientProgramEdit(){
    ...
    const s=D.createElement('script');
    s.src='trainer-client-program-edit.js?v=387';
    s.async=false;
    s.dataset.unvrslTrainerClientEditV262='1';
    s.onerror=()=>console.warn('UNVRSL trainer client program edit v262 failed to load');
    D.body?.appendChild(s);
  }
  function loadProgramWeekRpeRir(){
    ...
    const s=D.createElement('script');
    s.src='program-week-rpe-rir.js?v=387';
    s.async=false;
    s.dataset.unvrslWeekRpeRirV263='1';
    s.onerror=()=>console.warn('UNVRSL week RPE RIR v263 failed to load');
    D.body?.appendChild(s);
  }
  function loadProgramRepRange(){
    ...
    const s=D.createElement('script');
    s.src='program-rep-range.js?v=387';
    s.async=false;
    s.dataset.unvrslProgramRepRangeV266='1';
    s.onerror=()=>console.warn('UNVRSL program rep range v266 failed to load');
    D.body?.appendChild(s);
  }
  function loadBuiltInPlanRepRanges(){
    ...
    const s=D.createElement('script');
    s.src='built-in-plan-rep-ranges.js?v=387';
    s.async=false;
    s.dataset.unvrslBuiltInRangesV267='1';
    s.onerror=()=>console.warn('UNVRSL built-in plan rep ranges v267 failed to load');
    D.body?.appendChild(s);
  }
  function loadProgramWeekRepGuidance(){
    ...
    const s=D.createElement('script');
    s.src='program-week-rep-guidance.js?v=387';
    s.async=false;
    s.dataset.unvrslWeekRepGuidanceV268='1';
    s.onerror=()=>console.warn('UNVRSL weekly rep guidance v268 failed to load');
    D.body?.appendChild(s);
  }
  ...
  function warmFeatureModules(){
    ...
    featureModulesStarted=true;
    const loadAll=()=>{
      loadTrainingLoadModel();
      loadProgramIntensity();
      loadTrainerClientProgramEdit();
      loadProgramWeekRpeRir();
      loadProgramRepRange();
      loadBuiltInPlanRepRanges();
      loadProgramWeekRepGuidance();
    };
    loadAll();
    [1200,3000].forEach(ms=>setTimeout(loadAll,ms));
  }
  ...
  function warmFeatureIntent(e){
    const button=e.target?.closest?.('.nav button[data-p]');
    if(!button)return;
  ...
  // Root fix: critical startup = DOM + canonical ui-stability only.
  // Premium UI, stats, mobile patches, client nav and trainer shell are runtime
  // enhancements and must never keep the user behind the splash screen.
  ...
  function syncProgress(){
    let next=10;
    if(D.readyState!=='loading')next=22;
    if(W.__unvrslUiStabilityV316)next=Math.max(next,84);
    if(next>=84&&(cloudSettled()||elapsed()>=AUTH_GRACE_MS||(!cachedAuth()&&elapsed()>=CLOUD_GRACE_MS)))next=Math.max(next,91);
    if(next>=91&&(!client()||W.__unvrslClientRuntimeSettledV260))next=Math.max(next,94);
    setProgress(Math.min(next,96));
  }
  ...
  async function paintFinalInterface(){
    ...
    try{W.render?.()}catch(e){console.warn('UNVRSL final render v388',e)}
  ...
  async function finalize(){
    ...
    try{
      ...
      released=true;
      clearInterval(poll);
      W.dispatchEvent?.(new CustomEvent('unvrsl:app-ready',{detail:{release:RELEASE,queuedRender:pending,cloudBypassed:!!W.__unvrslStartupCloudBypassedV260}}));
      return true;
    }finally{
      finalizing=false;
    }
  ...
  for(const name of ['load','unvrsl:cloud-ready','unvrsl:client-ready','unvrsl:client-settled','unvrsl:readiness-ready','unvrsl:ui-stability-ready']){
  ...
  const poll=setInterval(finalize,80);
  finalize();
})();
