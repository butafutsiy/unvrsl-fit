...
async function cloudSignOut(){if(cloud.client)await cloud.client.auth.signOut();cloud.user=null;cloud.profile=null;closeModal();renderCloudAffordances();render()}
async function cloudSetRole(role){if(!cloud.user)return;const {data,error}=await cloud.client.from('profiles').update({role,updated_at:new Date().toISOString()}).eq('id',cloud.user.id).select().single();if(error)return alert(error.message);cloud.profile=data;closeModal();renderCloudAffordances();render();toast(role==='trainer'?'Режим тренера включён':'Режим клиента включён')}
...
async function cloudSaveName(){const display_name=$('#cloudName')?.value.trim();if(!display_name)return;const {data,error}=await cloud.client.from('profiles').update({display_name,updated_at:new Date().toISOString()}).eq('id',cloud.user.id).select().single();if(error)return alert(error.message);cloud.profile=data;closeModal();renderCloudAffordances();render()}
...
async function cloudSyncSession(s){if(!cloud.ready||!cloud.user||!s)return;try{await cloud.client.from('workouts').upsert({user_id:cloud.user.id,external_id:String(s.id),workout_date:s.date||iso(),payload:s,avg_rpe:cloudAvgRpe(s),completed_sets:done(s),total_sets:total(s),updated_at:new Date().toISOString()},{onConflict:'user_id,external_id'})}catch(e){console.warn('sync workout',e)}}
async function cloudSyncBodyweights(){if(!cloud.ready||!cloud.user)return;const rows=(st.bw||[]).filter(x=>x.d&&x.w).map(x=>({user_id:cloud.user.id,measure_date:x.d,weight_kg:Number(x.w)}));if(rows.length)await cloud.client.from('bodyweights').upsert(rows,{onConflict:'user_id,measure_date'})}
async function cloudSyncAll(){if(!cloud.user)return cloudAccountSheet();toast('Синхронизация…');for(const s of st.sessions||[])await cloudSyncSession(s);await cloudSyncBodyweights();toast('Синхронизировано')}
