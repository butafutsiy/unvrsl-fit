...
(()=>{
  ...
  function mark(name){ready[name]=true;if(Object.values(ready).every(Boolean)){window.__unvrslReadinessStackReadyV257=true;window.__unvrslReadinessStackReadyV260=true;window.dispatchEvent(new CustomEvent('unvrsl:readiness-ready',{detail:{release:301}}))}}
  ...
  let attempts=0;
  function load(){
    loadBuiltinProfile();loadSergeyProfile();loadRecommendationCopy();
    ...
    s.onload=()=>{if(window.__unvrslTrainingEngineV257){mark('engine');loadPrescriptionBridge();loadBuiltinProfile();loadSergeyProfile();loadRecommendationCopy()}else setTimeout(load,250)};
  ...
  loadBuiltinProfile();loadSergeyProfile();loadRecommendationCopy();load();loadQuestionnaire();loadExactPlanFix();loadWorkoutShare();loadPrescriptionBridge();
  ['unvrsl:training-engine-ready','unvrsl:modules-ready','unvrsl:app-ready','unvrsl:cloud-modules-settled'].forEach(ev=>window.addEventListener(ev,()=>{loadBuiltinProfile();loadSergeyProfile();loadRecommendationCopy();loadPrescriptionBridge()},{passive:true}));
})();
