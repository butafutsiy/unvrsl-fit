...
(()=>{
  ...
  style.textContent=`
    #start .u177-rec{margin:4px 0 1px 28px!important;min-height:16px;display:flex;align-items:center;gap:5px;flex-wrap:wrap;font-size:10.5px;line-height:1.15;color:#8e8e93}
  ...
  function recommendationFor(e,x,cur){
    if(!x)return null;
    const adaptive=num(x.adaptive174Suggested??x.adaptiveSuggestedW);
    const existing=num(x.recommended177W);
    const planned=num(x.adaptive174From??x.plannedW);
    const current=num(x.w);
    const w=adaptive??existing??planned??current;
    if(!(w>0))return null;
    const reps=num(x.r),target=num(e?.target)??num(cur?.target);
    const adaptiveMode=adaptive!=null&&adaptive>0;
  ...
  function render(){
    ...
    document.querySelectorAll('#start .exercise:not(.anton-superset):not(.anton-single) .setrow:not(.cardiorow)').forEach(row=>{
      ...
      if(!el){el=document.createElement('div');el.className='u177-rec';el.dataset.ei=idx.ei;el.dataset.si=idx.si;const prev=row.nextElementSibling?.classList?.contains('prev-set')?row.nextElementSibling:null;if(prev)prev.insertAdjacentElement('afterend',el);else row.insertAdjacentElement('afterend',el)}
      el.dataset.mode=rec.mode;
      const effort=rec.target!=null?` · RPE ${fmt(rec.target)}`:'',reps=rec.reps!=null?` × ${fmt(rec.reps)}`:'';
      const mode=rec.mode==='adaptive'?'адаптивно':'по плану';
      el.innerHTML=`<span>Рекомендация:</span> <b>${fmt(rec.w)} кг${reps}${effort}</b><span class="u177-mode">${mode}</span>`;
    });
  }

  function install(){
    let base=null;try{base=typeof startPage==='function'?startPage:window.startPage}catch(_){base=window.startPage}
    ...
    try{startPage=wrapped}catch(_){window.startPage=wrapped}
    return true;
  }

  try{install();[50,250,900,1800].forEach(t=>setTimeout(()=>{try{install();render()}catch(e){console.warn('recommendation v177',e)}},t));const root=document.getElementById('start');if(root)new MutationObserver(()=>queueMicrotask(()=>{try{render()}catch(_){}})).observe(root,{childList:true,subtree:true});render()}catch(e){console.warn('recommendation v177 init',e)}
})();
