...
(()=>{
  ...
  .nav{position:fixed;left:10px;right:10px;bottom:8px;z-index:1000;display:grid!important;grid-template-columns:repeat(var(--nav-cols,5),minmax(0,1fr))!important;align-items:end;gap:0;padding:10px 3px calc(9px + env(safe-area-inset-bottom));border:1px solid #343438;border-radius:26px;background:rgba(25,25,27,.96);box-shadow:0 16px 50px rgba(0,0,0,.55);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);overflow:visible!important}
  ...
  .nav .start .ico svg{width:29px;height:29px;fill:currentColor;stroke:none}
  ...
  const icons={
    ...
    programs:'<svg viewBox="0 0 24 24"><circle cx="5" cy="6" r="1"/><circle cx="5" cy="12" r="1"/><circle cx="5" cy="18" r="1"/><path d="M9 6h11M9 12h11M9 18h11"/></svg>',
    start:'<svg viewBox="0 0 24 24"><path d="m8 5 11 7-11 7Z"/></svg>',
