...
(()=>{
  ...
  function today(v,ex,cur){const n=N(v);if(n==null)return null;try{const fn=W.unvrslTrainingReadinessWeightV292;if(typeof fn==='function')return fn(n,ex,cur,stepFor(ex))}catch(_){ }const f=cur?.trainingReadinessDone&&cur?.readinessAdjusted?(N(cur?.readiness?.factor)||1):1;const s=stepFor(ex);return Math.max(0,Math.round((n*f)/s)*s)}
  ...
  function v292Ready(group,cur){
    if(Number(cur?.trainingLoadModelRevision)!==292)return false;
    return group.some(ex=>(ex?.set||[]).some(s=>N(s?.recommendedW)!=null&&!!s?.trainingIntensity292));
  }
  function reasonFor(group){
    const progression=group.map(x=>x?.trainingProgression292).find(Boolean);
    if(progression?.reason)return progression.reason;
    const estimate=group.map(x=>x?.trainingEstimate200).find(Boolean);
    if(estimate?.confidence)return`Расчёт v292 · уверенность ${estimate.confidence}`;
    return'Единая рекомендация v292';
  }
  function removeLegacy(root){
    root?.querySelectorAll('.te200-rec,.smart-suggest,.u177-rec,.wr180,.wr185,.adaptive-rec,[data-legacy-recommendation]').forEach(x=>x.remove());
  }
  function render(){
    ...
    (cur.ex||[]).forEach((ex,i)=>{
      ...
      let el=card.querySelector(`.tlm292-rec[data-rec-id="${CSS.escape(id)}"]`);
      ...
      const applied=group.some(g=>g.weightDecision==='recommendation'),rp=readinessPercent(cur),reason=reasonFor(group);
      el.innerHTML=`<div class="tlm292-rec-main"><b>Рекомендация · ${fmt(recToday)} кг</b><span>${reason} · план сегодня ${fmt(planToday)} кг${rp?` · самочувствие ${rp}`:''}</span></div><button type="button">${applied?'Вернуть план':'Применить'}</button>`;
      el.querySelector('button').onclick=()=>{try{applied?W.trainingRestoreProgram200?.(k):W.trainingApplyRecommendation200?.(k)}finally{setTimeout(render,0)}};
    });
