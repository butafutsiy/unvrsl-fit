...
(()=>{
  ...
  function render(updatedAt){
    ...
    requestAnimationFrame(()=>root.querySelectorAll('.pp-point-scroll').forEach(x=>{x.scrollLeft=x.scrollWidth}))
  }
  ...
  W.publicProgressChartPointV326=function(id,index){
    const graph=D.getElementById(String(id||'')),i=Number(index);if(!graph||!Number.isInteger(i))return;
    graph.querySelectorAll('[data-pp-dot]').forEach(x=>x.classList.toggle('is-selected',Number(x.dataset.ppDot)===i));
    graph.querySelectorAll('[data-pp-value]').forEach(x=>x.classList.toggle('is-selected',Number(x.dataset.ppValue)===i));
    ...
    const chip=graph.querySelector(`[data-pp-chip="${i}"]`),focus=graph.querySelector('[data-pp-focus]'),scroller=graph.querySelector('.pp-point-scroll');if(chip&&focus)focus.textContent=chip.dataset.ppLabel||'';
    if(chip&&scroller){const left=chip.offsetLeft-(scroller.clientWidth-chip.offsetWidth)/2;scroller.scrollTo({left:Math.max(0,left),behavior:'smooth'})}
  };
