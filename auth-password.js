...
window.cloudPasswordLoginSheet=function(){
 const saved=authSavedLogin(),notice=cloud?.degraded?`<div class="card" style="margin:14px 0"><b>Есть проблема со связью с облаком</b><div class="muted small" style="margin-top:5px">Локальные данные приложения сохранены. Можно попробовать войти повторно.</div></div>`:'';
...
window.cloudSignInPassword=async function(){
 ...
 try{
  ...
  authRememberLogin(email);cloud.user=data?.user||data?.session?.user||null;cloud.profile=null;if(typeof cloudMarkHealthy==='function')cloudMarkHealthy();closeModal();renderCloudAffordances();try{render()}catch(_){ }toast('Вход выполнен');if(cloud.user&&typeof cloudDeferProfile==='function')cloudDeferProfile('password-signin');
...
window.cloudAccountSheet=function(){
 ...
 if(!cloud?.user)return cloudPasswordLoginSheet();
 const trainer=(typeof unvrslTrainerMode==='function'&&unvrslTrainerMode())||cloud.profile?.role==='trainer',role=trainer?'Тренер':'Клиент',offline=cloud?.degraded?`<div class="card" style="margin-top:14px"><b>Облако временно недоступно</b><div class="muted small" style="margin-top:5px">Приложение продолжит работать с данными на устройстве. Облачная синхронизация восстановится автоматически.</div></div>`:'';
