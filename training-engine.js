...
(()=>{
 ...
 async function ensureLoadModel(){
  ...
  if(!document.querySelector('script[data-unvrsl-load-model-v292]')){const s=document.createElement('script');s.src='training-load-model.js?v=410';s.async=false;s.dataset.unvrslLoadModelV292='1';document.body.appendChild(s)}
 ...
 function enhanceDom(){const cur=W.st?.current,root=document.getElementById('start');if(!cur||!root)return;const sig=domSignature(cur);if(root.dataset.te200Sig===sig&&root.querySelector('.te200-readiness'))return;root.querySelectorAll('.te200-rec,.te200-auto,.te200-readiness,.te405-next').forEach(x=>x.remove());const cards=[...root.querySelectorAll('.exercise')],seen=new Set();
  ...
  groups.forEach((group,index)=>{const ex=group.entries[0],card=cards[index];if(!card||['time','distance'].includes(WorkoutDomain.loadType(ex,workoutRegistry)))return;const pending=group.entries.flatMap(e=>(e.set||[]).filter(s=>!s.ok).map(s=>({e,s}))),item=pending.find(x=>x.s.recommendation);if(!item)return;
   const first=ex.set?.[0],standard=WorkoutDomain.method(ex,first)==='STANDARD';
   const rec=standard&&first?(first.ok?WorkoutDomain.recommend(ex,first,cur,st.sessions||[],workoutRegistry,st.exerciseWeightProfiles||{}):first.recommendation)||item.s.recommendation:item.s.recommendation;
   ...
   const showHint=()=>{if(!liveHint)return;const hint=document.createElement('div');hint.className='te405-next';hint.style.cssText='padding:9px 11px;color:var(--muted);font-size:12px';hint.textContent=`Следующий подход: ${String(liveHint.weight).replace('.',',')} кг · ${liveHint.reason}. Вес меняется вручную.`;el.insertAdjacentElement('afterend',hint)};
   if(!rec.sessionIds?.length){el.textContent=group.entries.some(e=>e.programWeightMode==='prescribed')?'Плановый вес сохранён. После первого выполненного подхода запиши повторы и RPE – следующая тренировка получит рекомендацию.':'С этим оборудованием пока нет истории – укажи комфортный вес, затем фактические повторы и RPE. После тренировки появится рекомендация.';card.querySelector('.row')?.insertAdjacentElement('afterend',el);showHint();return}
   const fmt=v=>String(v).replace('.',','),range=rec.repRange,repText=range.lo===range.hi?`${range.lo}`:`${range.lo}–${range.hi}`;
   const type=WorkoutDomain.loadType(ex,workoutRegistry),weightText=type==='bodyweight_only'?'свой вес':type==='per_dumbbell'?`${fmt(rec.weight)} кг на гантель`:type==='per_side'?`${fmt(rec.weight)} кг на сторону`:type==='bodyweight_assisted'?`${fmt(rec.weight)} кг помощи`:`${fmt(rec.weight)} кг`;
   const title=document.createElement('div');title.className='te200-rec-main';const b=document.createElement('b');b.textContent=`${rec.planPreserved?'План сохранён, нет надёжной рекомендации':'Рекомендация на упражнение'}: ${weightText} · ${repText} повт.`;title.append(b);
   ...
   const detail=document.createElement('details'),summary=document.createElement('summary');summary.textContent='Почему этот вес?';detail.append(summary);const text=document.createElement('p');text.className='small muted';text.textContent=`${rec.reason}. Цель: ${repText} повт., RPE ${fmt(first?.targetRpeMin??ex.targetRpeMin??cur.programWeekRpeMin??cur.target??7)}–${fmt(first?.targetRpeMax??ex.targetRpeMax??cur.programWeekRpeMax??cur.target??8)}. ${rec.previous==null?'Нет прошлой сопоставимой нагрузки':`Изменение: ${rec.delta>0?'+':''}${fmt(rec.delta)} кг`}. Шаг: ${fmt(rec.step)} кг. Уверенность: ${rec.confidence}. ${rec.rounding}. Прошлые подходы: ${rec.evidence.join('; ')||'нет'}`;detail.append(text);title.append(detail);el.append(title);
   if(!rec.planPreserved&&group.entries.some(e=>e.programWeightMode==='prescribed')){const button=document.createElement('button');button.type='button';button.className='te200-rec-apply';button.textContent='Применить';button.onclick=()=>{const base=N(first?.programW)??N(first?.plannedW)??N(first?.w),delta=standard&&base>0?rec.weight-base:null;for(const {e,s} of pending){if(s.manualOverride||s.weightSource==='manual'||!s.recommendation)continue;let chosen=s.recommendation;if(delta!=null){const initial=N(s.programW)??N(s.plannedW)??N(s.w);if(initial!=null&&initial>0)chosen={...chosen,weight:WorkoutDomain.roundWeight(initial+delta,WorkoutDomain.profile(e,workoutRegistry,st.exerciseWeightProfiles||{}))}}WorkoutDomain.applyAuto(s,chosen);s.plannedW=s.w;e.weightDecision='recommendation'}W.save();W.startPage()};el.append(button)}
