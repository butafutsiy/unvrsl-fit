'use strict';
(()=>{
 const W=window,REV=295,MATH_OWNER='workout-domain';
 ...
 .te200-scale{display:flex;justify-content:space-between;color:#777;font-size:10px;margin-top:3px}.te200-explicit{margin-top:10px;color:#777;font-size:11px;line-height:1.4}.te406-basis,.te410-week{font-size:11px;color:#a0a0a6;margin:5px 0;line-height:1.4}.te410-week{color:#c8a3ff}.te410-equipment{font-size:10px;color:#85858b;margin-top:4px}
 ...
 function step(ex){return WorkoutDomain.profile(ex,workoutRegistry,st.exerciseWeightProfiles||{}).step}
 function round(v,s){return WorkoutDomain.roundWeight(v,{step:s,min:0})}
 ...
 async function ensureLoadModel(){
  ...
  if(!document.querySelector('script[data-unvrsl-load-model-v292]')){const s=document.createElement('script');s.src='training-load-model.js?v=434';s.async=false;s.dataset.unvrslLoadModelV292='1';document.body.appendChild(s)}
 ...
 async function prepare(cur){
  ...
  lockLegacy(cur);W.equipmentAttachDefaults405?.(cur);clearRecommendationState(cur);
  for(const ex of cur.ex||[]){
   if(['time','distance'].includes(WorkoutDomain.loadType(ex,workoutRegistry)))continue;const src=source(ex,cur);if((cur.programId||cur.planId)&&!src){unresolved=true;continue}
   ...
   const mode=ownerPlan||src?.weightMode==='manual'||sourceWeights.some(x=>x>0)?'prescribed':'adaptive';
 ...
 function domSignature(cur){return JSON.stringify([cur.id,cur.trainingWeightPolicy214,cur.trainingLoadModelRevision||0,cur.trainingLoadModelAt||'',!!cur.trainingReadinessDone,!!cur.readinessAdjusted,cur.readiness?.score??null,(cur.ex||[]).map(ex=>[key(ex),ex.programWeightMode,ex.weightDecision,ex.trainingProgression292?.action||'',ex.trainingProgression292?.reason||'',(ex.set||[]).map(s=>[num(s.programW),num(s.recommendedW),!!s.trainingIntensity292,num(s.plannedW),num(s.w),!!s.ok,s.actualReps,s.actualRpe,s.recommendation?.repRange,s.recommendation?.basis?.date,s.recommendation?.excludedHistory,s.recommendation?.nextSetSuggestion?.weight])])])}
 function enhanceDom(){const cur=W.st?.current,root=document.getElementById('start');if(!cur||!root)return;const sig=domSignature(cur);if(root.dataset.te200Sig===sig&&root.querySelector('.te200-readiness'))return;root.querySelectorAll('.te200-rec,.te200-auto,.te200-readiness,.te405-next').forEach(x=>x.remove());const cards=[...root.querySelectorAll('.exercise')],seen=new Set();
  const groups=typeof groupIndexedEntries==='function'?groupIndexedEntries(cur.ex):cur.ex.map((e,i)=>({entries:[e],indices:[i]}));
  groups.forEach((group,index)=>{const ex=group.entries[0],card=cards[index];if(!card||['time','distance'].includes(WorkoutDomain.loadType(ex,workoutRegistry)))return;const pending=group.entries.flatMap(e=>(e.set||[]).filter(s=>!s.ok).map(s=>({e,s}))),item=pending.find(x=>x.s.recommendation);if(!item)return;
   const rec=item.s.recommendation;
   const liveHint=item.s.recommendation?.nextSetSuggestion;
   const el=document.createElement('div');el.className='te200-rec';el.dataset.recommendationOwner=MATH_OWNER;
   const showHint=()=>{if(!liveHint)return;const hint=document.createElement('div');hint.className='te405-next';hint.style.cssText='padding:9px 11px;color:var(--muted);font-size:12px';hint.textContent=`Следующий подход: ${String(liveHint.weight).replace('.',',')} кг · ${liveHint.reason}. ${item.e.programWeightMode==='adaptive'?'Вес подставится автоматически.':'Нажми «Применить», чтобы изменить вес.'}`;el.insertAdjacentElement('afterend',hint)};
   if(!rec.sessionIds?.length&&!rec.nextSetSuggestion){el.textContent=rec.excludedHistory?`Тренировка ${rec.excludedHistory.date} не объединена с текущей: ${rec.excludedHistory.reason}. Проверь оборудование или тип нагрузки.`:group.entries.some(e=>e.programWeightMode==='prescribed')?'Плановый вес сохранён. После первого выполненного подхода запиши повторы и RPE – следующая тренировка получит рекомендацию.':'С этим оборудованием пока нет истории – укажи комфортный вес, затем фактические повторы и RPE. После тренировки появится рекомендация.';card.querySelector('.row')?.insertAdjacentElement('afterend',el);showHint();return}
   const fmt=v=>String(v).replace('.',','),range=rec.repRange,repText=range.lo===range.hi?`${range.lo}`:`${range.lo}–${range.hi}`;
   const type=WorkoutDomain.loadType(ex,workoutRegistry),suggested=rec.nextSetSuggestion?.weight??rec.weight,weightText=type==='bodyweight_only'?'свой вес':type==='per_dumbbell'?`${fmt(suggested)} кг на гантель`:type==='per_side'?`${fmt(suggested)} кг на сторону`:type==='bodyweight_assisted'?`${fmt(suggested)} кг помощи`:`${fmt(suggested)} кг`;
   const title=document.createElement('div');title.className='te200-rec-main';const b=document.createElement('b');b.textContent=`${rec.planPreserved?'План сохранён, нужна проверка':rec.nextSetSuggestion?'Следующий подход':'Рекомендация на подход'}: ${weightText} · ${repText} повт.`;title.append(b);
   if(rec.basis){const basis=document.createElement('div');basis.className='te406-basis';basis.textContent=`Основа: ${String(rec.basis.date||'прошлая тренировка').slice(0,10)} · самый тяжёлый рабочий сет ${fmt(rec.basis.weight)} кг${rec.basis.estimatedOneRepMax?` · 1ПМ ≈ ${fmt(rec.basis.estimatedOneRepMax)} кг`:''}${rec.basis.planned?` · план ${fmt(rec.basis.planned)} кг`:''}`;title.append(basis)}
   if(rec.excludedHistory){const skip=document.createElement('div');skip.className='te410-week';skip.textContent=`Есть более новая тренировка ${rec.excludedHistory.date}, но она не объединена с этой: ${rec.excludedHistory.reason}. Проверь настройки упражнения и оборудования.`;title.append(skip)}
   if(rec.weeklyIntensity){const week=document.createElement('div');week.className='te410-week';const wi=rec.weeklyIntensity;week.textContent=`Неделя: ${fmt(wi.min)}–${fmt(wi.max)}%${wi.estimatedMin!=null?` · коридор ≈ ${fmt(wi.estimatedMin)}–${fmt(wi.estimatedMax)} кг`:''}${wi.applied?' · учтено в весе':' · диапазон повторений имеет приоритет'}`;title.append(week)}
   if(rec.equipmentName){const equipment=document.createElement('div');equipment.className='te410-equipment';equipment.textContent=`Оборудование: ${rec.equipmentName} · история считается отдельно`;title.append(equipment)}
   const detail=document.createElement('details'),summary=document.createElement('summary');summary.textContent='Почему этот вес?';detail.append(summary);const text=document.createElement('p');text.className='small muted';text.textContent=`${rec.reason}. Цель: ${repText} повт., RPE ${fmt(item.s?.targetRpeMin??ex.targetRpeMin??cur.programWeekRpeMin??cur.target??7)}–${fmt(item.s?.targetRpeMax??ex.targetRpeMax??cur.programWeekRpeMax??cur.target??8)}. ${rec.nextSetSuggestion?`После подхода сегодня: ${rec.nextSetSuggestion.reason}, ${fmt(rec.nextSetSuggestion.weight)} кг. `:''}${rec.previous==null?'Нет прошлой сопоставимой нагрузки':`Изменение: ${rec.delta>0?'+':''}${fmt(rec.delta)} кг`}. Шаг: ${fmt(rec.step)} кг. Уверенность: ${rec.confidence}. ${rec.rounding}. Прошлые подходы: ${rec.evidence.join('; ')||'нет'}`;detail.append(text);title.append(detail);el.append(title);
   if(!rec.planPreserved&&group.entries.some(e=>e.programWeightMode==='prescribed')){const button=document.createElement('button');button.type='button';button.className='te200-rec-apply';button.textContent='Применить';button.onclick=()=>{for(const {e,s} of pending){if(s.manualOverride||s.weightSource==='manual'||s.recommendation?.planPreserved||N(s.recommendedW)==null)continue;WorkoutDomain.applyAuto(s,{...s.recommendation,weight:N(s.recommendedW)});s.plannedW=s.w;e.weightDecision='recommendation'}W.save();W.startPage()};el.append(button)}
   card.querySelector('.row')?.insertAdjacentElement('afterend',el);
   showHint();
  });
 ...
 function installStart(name){const fn=W[name];if(typeof fn!=='function'||fn.__te205PreStart)return;const wrapped=function(){if(W.st?.current){W.nav?.('start');W.toast?.('У вас есть незавершённая тренировка');return}if(startingAfterReadiness)return fn.apply(this,arguments);askBeforeStart(fn,arguments,this)};wrapped.__te205PreStart=true;wrapped.__te205Base=fn;assignStart(name,wrapped)}
