...
(()=>{
 ...
 .te200-scale{display:flex;justify-content:space-between;color:#777;font-size:10px;margin-top:3px}.te200-explicit{margin-top:10px;color:#777;font-size:11px;line-height:1.4}.te406-basis{font-size:11px;color:#a0a0a6;margin:5px 0;line-height:1.4}
 ...
 async function ensureLoadModel(){
  ...
  if(!document.querySelector('script[data-unvrsl-load-model-v292]')){const s=document.createElement('script');s.src='training-load-model.js?v=407';s.async=false;s.dataset.unvrslLoadModelV292='1';document.body.appendChild(s)}
 ...
 function enhanceDom(){const cur=W.st?.current,root=document.getElementById('start');if(!cur||!root)return;const sig=domSignature(cur);if(root.dataset.te200Sig===sig&&root.querySelector('.te200-readiness'))return;root.querySelectorAll('.te200-rec,.te200-auto,.te200-readiness,.te405-next').forEach(x=>x.remove());const cards=[...root.querySelectorAll('.exercise')],seen=new Set();
  ...
  groups.forEach((group,index)=>{const ex=group.entries[0],card=cards[index];if(!card||['time','distance'].includes(WorkoutDomain.loadType(ex,workoutRegistry)))return;const pending=group.entries.flatMap(e=>(e.set||[]).filter(s=>!s.ok).map(s=>({e,s}))),item=pending.find(x=>x.s.recommendation);if(!item)return;
   ...
   if(rec.basis){const basis=document.createElement('div');basis.className='te406-basis';basis.textContent=`Основа: ${String(rec.basis.date||'прошлая тренировка').slice(0,10)} · самый тяжёлый рабочий сет ${fmt(rec.basis.weight)} кг${rec.basis.planned?` · план ${fmt(rec.basis.planned)} кг`:''}`;title.append(basis)}
