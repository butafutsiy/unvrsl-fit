...
(()=>{
 ...
 function lockLegacy(cur){if(!cur)return;cur.unvrslAdaptive174Applied=true;cur.adaptiveEffortV2Applied=true;cur.adaptiveDecision='engine210';cur.adaptivePrompted=true;delete cur.trainingWeightChoice;delete cur.engine196Prepared;delete cur.engine196FlowShown;delete cur.weightsPrepared194}
 ...
 async function confirm(adjust){
  ...
  if(pendingStart){
   ...
   try{await prepare(cur);enhanceDom()}catch(e){console.warn('v210 prepare before reveal',e);W.toast?.('Не удалось подготовить веса')}finally{busy=false;document.documentElement?.classList?.remove('te200-preparing');try{W.closeModal?.()}catch(_){}}
