...
(()=>{
 ...
 function disableLegacyReadiness(){if(typeof W.advAskReadiness!=='function'||W.advAskReadiness.__te204)return;const bypass=(fn,args)=>typeof fn==='function'?fn.apply(W,Array.isArray(args)?args:[]):undefined;bypass.__te204=true;W.advAskReadiness=bypass;try{advAskReadiness=bypass}catch(_){}}
 ...
 function todayWeight(v,ex,cur){if(!(num(v)>0))return null;const f=cur?.trainingReadinessDone&&cur?.readinessAdjusted?num(cur?.readiness?.factor)||1:1;return round(num(v)*f,step(ex))}
 ...
 function readiness(){const g=id=>num(document.getElementById(id)?.value||3),sl=g('te200Sleep'),en=g('te200Energy'),so=g('te200Sore'),st=g('te200Stress'),pos=v=>(v-1)/4,neg=v=>(5-v)/4,score=Math.round(pos(sl)*25+pos(en)*30+neg(so)*30+neg(st)*15);let factor=1;if(score<20)factor=.90;else if(score<35)factor=.925;else if(score<50)factor=.95;else if(score>=92)factor=1.025;return{sleep:sl,energy:en,soreness:so,stress:st,score,factor,skipped:false,at:new Date().toISOString()}}
 function item(id,label,left,right){return `<div class="te200-item"><b><span>${label}</span><span id="${id}V">3</span></b><input id="${id}" data-touched="0" type="range" min="1" max="5" value="3" oninput="this.dataset.touched='1';document.getElementById('${id}V').textContent=this.value"><div class="te200-scale"><span>${left}</span><span>${right}</span></div></div>`}
 function showReadiness(){const cur=W.st?.current;if(!cur)return;cur.trainingReadinessPromptShown=true;try{W.save?.()}catch(_){}W.modal?.(`<div data-te200-flow="readiness"><div class="sheet-grabber"></div><h2>Самочувствие перед тренировкой</h2><div class="muted">Сначала рассчитывается базовый вес: из программы или по прошлой тренировке. Коррекция самочувствия применяется к нему только после твоего выбора.</div><div class="te200-ready">${item('te200Sleep','Сон','плохо','отлично')}${item('te200Energy','Энергия','низкая','высокая')}${item('te200Sore','Крепатура','нет','сильная')}${item('te200Stress','Стресс','низкий','высокий')}</div><button class="btn primary full" onclick="trainingConfirmReadiness200(true)">Учесть самочувствие</button><button class="btn full" style="margin-top:10px" onclick="trainingConfirmReadiness200(false)">Оставить базовый вес</button><div class="te200-explicit">Чтобы учесть самочувствие, оцени все четыре показателя. Значения по умолчанию сами ничего не меняют.</div></div>`)}
 function confirm(adjust){const cur=W.st?.current;if(!cur)return;if(adjust){const fields=['te200Sleep','te200Energy','te200Sore','te200Stress'].map(id=>document.getElementById(id));if(fields.some(x=>!x||x.dataset.touched!=='1')){W.toast?.('Оцени все четыре показателя');return}}const d=adjust?readiness():{sleep:null,energy:null,soreness:null,stress:null,score:null,factor:1,skipped:true,at:new Date().toISOString()};cur.readiness=d;cur.readinessUsed=!!adjust;cur.readinessAdjusted=!!adjust&&Math.abs(d.factor-1)>.001;cur.trainingReadinessDone=true;(cur.ex||[]).forEach(ex=>{(ex.set||[]).forEach(s=>{if(s.ok||s.manualOverride||num(s.plannedW)<=0)return;s.w=todayWeight(s.plannedW,ex,cur)})});W.st.readinessLog.push({date:cur.date,sessionId:cur.id,...d});try{W.save?.();W.startPage?.();W.closeModal?.()}catch(_){}setTimeout(enhanceDom,0)}
