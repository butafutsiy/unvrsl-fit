...
(()=>{
 ...
 function readinessMarkup(){return `<div data-te200-flow="readiness"><div class="sheet-grabber"></div><h2>Самочувствие перед тренировкой</h2><div class="muted">После ответа откроется тренировка. Вес будет рассчитан из программы или по прошлой тренировке, затем скорректирован по самочувствию.</div><div class="te200-ready">${item('te200Sleep','Сон','плохо','отлично')}${item('te200Energy','Энергия','низкая','высокая')}${item('te200Sore','Крепатура','нет','сильная')}${item('te200Stress','Стресс','низкий','высокий')}</div><button class="btn primary full" onclick="trainingConfirmReadiness200(true)">Учесть и открыть тренировку</button><button class="btn full" style="margin-top:10px" onclick="trainingConfirmReadiness200(false)">Открыть без коррекции</button><div class="te200-explicit">Чтобы учесть самочувствие, оцени все четыре показателя. Значения по умолчанию сами ничего не меняют.</div></div>`}
