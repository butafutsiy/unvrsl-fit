...
(()=>{
 ...
 function lockLegacy(cur){if(!cur)return;cur.unvrslAdaptive174Applied=true;cur.adaptiveEffortV2Applied=true;cur.adaptiveDecision='engine214';cur.adaptivePrompted=true;delete cur.trainingWeightChoice;delete cur.engine196Prepared;delete cur.engine196FlowShown;delete cur.weightsPrepared194}
 ...
 async function prepare(cur){
  ...
  for(const ex of cur.ex||[]){
   ...
   const seeds=sets.map((s,i)=>{const fromProgram=src?sourceWeight(src,ex,i,cur):0;return fromProgram>0?fromProgram:num(launch[i])});
   const mode=ownerPlan?'prescribed':'adaptive';ex.programWeightMode=mode;delete ex.recommendation194;delete ex.engine196Recommendation;delete ex.progression187;
   if(mode==='adaptive'){
 ...
 function domSignature(cur){return JSON.stringify([cur.id,cur.trainingWeightPolicy214,!!cur.trainingReadinessDone,!!cur.readinessAdjusted,cur.readiness?.score??null,(cur.ex||[]).map(ex=>[key(ex),ex.programWeightMode,ex.weightDecision,(ex.set||[]).map(s=>[num(s.programW),num(s.recommendedW),num(s.plannedW),num(s.w)])])])}
 function enhanceDom(){const cur=W.st?.current,root=document.getElementById('start');if(!cur||!root)return;root.querySelectorAll('.smart-suggest,.u177-rec,.wr180,.wr185').forEach(x=>x.remove());const sig=domSignature(cur);if(root.dataset.te200Sig===sig&&root.querySelector('.te200-readiness'))return;root.querySelectorAll('.te200-rec,.te200-auto,.te200-readiness').forEach(x=>x.remove());const cards=[...root.querySelectorAll('.exercise')];const seen=new Set(),ownerPlan=isOwnerEightWeekPlan(cur);
