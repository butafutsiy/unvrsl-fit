...
(()=>{
  ...
  if(W.__unvrslNativePreviewRangesV278)return;
  W.__unvrslNativePreviewRangesV278=true;
  ...
  const routineFor=(w,c)=>{
    try{if(typeof rmap!=='undefined')return rmap.get(`${w}-${c}`)||null}catch(_){ }
    return (W.UNVRSL_ROUTINES||[]).find(r=>Number(r?.w)===Number(w)&&String(r?.c)===String(c))||null;
  };
  ...
  function patch(){
    ...
    if(typeof cur!=='function'||cur.__unvrslNativePreviewRangesV278)return false;
    const wrapped=function(w,c){
      ...
      (r.e||[]).forEach((e,i)=>{
        const label=labelFor(r.w,e);if(!label)return;
        const meta=rows[i]?.querySelector('.muted.small');if(!meta)return;
        const from=e.r;
        if(from==null)return;
        meta.textContent=meta.textContent.replace(new RegExp(`(×\\s*)${escRe(from)}(?=\\s|·|$)`),`$1${label}`);
      });
      return out;
    };
    wrapped.__unvrslNativePreviewRangesV278=true;
    wrapped.__unvrslNativePreviewRangesV278Base=cur;
    W.preview=wrapped;
