...
(()=>{
 ...
 function flowIs(kind){return document.getElementById('sheet')?.querySelector?.(`[data-te199-flow="${kind}"]`)}
 function showChoice(cur){if(flowIs('choice'))return;const list=awaiting(cur);if(!list.length)return showReadiness(cur);const groups=new Map();list.forEach(ex=>{const k=ex.sourceId?`id:${ex.sourceId}`:`n:${base(ex.n).toLowerCase()}`;if(!groups.has(k))groups.set(k,{name:base(ex.n),plan:[],rec:[]});const g=groups.get(k);(ex.set||[]).forEach(s=>{if(num(s.programW)>0)g.plan.push(num(s.programW));if(num(s.recommendedW)>0)g.rec.push(num(s.recommendedW))})});const html=[...groups.values()].map(g=>`<div class="te199-row"><div><b>${esc(g.name)}</b><span>План: ${g.plan.join(' / ')} кг</span></div><div style="text-align:right"><b>${g.rec.join(' / ')} кг</b><span>рекомендация</span></div></div>`).join('');W.modal?.(`<div data-te199-flow="choice"><div class="sheet-grabber"></div><h2>Рабочие веса</h2><div class="muted">Выбор показывается только для упражнений, где тренер заранее указал вес. Упражнения без исходного веса уже рассчитаны автоматически по прошлой тренировке.</div><div class="te199-list">${html}</div><button class="btn primary full" onclick="trainingChooseWeights199('recommendation')">Применить рекомендации</button><button class="btn full" style="margin-top:10px" onclick="trainingChooseWeights199('program')">Оставить веса программы</button></div>`)}
