...
(function(root,factory){
  ...
  function boot(W,A){
    ...
    function isCardio(group){
      const entries=group?.entries||[],name=String(group?.base||'').toLowerCase();
      if(A.isCardioName(name))return true;
      return entries.length>0&&entries.every(e=>e.mode==='cardio'||N(e.m)>0||N(e.minutes)>0||N(e.duration)>0||(e.set||[]).some(x=>N(x.m)>0||N(x.min)>0||N(x.minutes)>0||N(x.duration)>0))
    }
    ...
    function recommendation(group,profile){
      if(!group||!profile||isCardio(group))return null;
    ...
    function openPicker(token){
      ...
      const active=selected(group),recent=recentStore()[key],picked=active||recent,currentId=picked?.id||'',cardio=isCardio(group);
      ...
      const planCard=cardio?`<div class="mw304-plan"><span>Кардиооборудование</span><b>Название или модель</b><small>Для аэробайка вес и коэффициент не рассчитываются</small></div>`:`<div class="mw304-plan"><span>Исходный вес плана</span><b>${esc(planText)}${plan.length?' кг':''}</b><small>Он не изменяется при выборе тренажёра</small></div>`;
      const history=cardio?`<div class="mw304-first"><b>Без пересчёта веса</b><span>Название сохранится у упражнения, чтобы в следующий раз выбрать тот же аэробайк или кардиотренажёр.</span></div>`:sampleCount?`<div class="mw304-result"><b>${rec?`Рекомендация для этого тренажёра: ${esc(A.formatWeights(rec.weights))} кг`:'История тренажёра сохранена'}</b><span>Калибровок: ${sampleCount} · коэффициент ${String(rec?.ratio||A.median((profile.samples||[]).map(x=>x.ratio))||1).replace('.',',')}</span></div>`:`<div class="mw304-first"><b>Первая калибровка</b><span>Выставь рабочий вес вручную и выполни подходы в нужном RPE. После тренировки сайт запомнит вес именно для введённого названия.</span></div>`;
      W.modal?.(`<div class="sheet-grabber"></div><div class="row between"><div><h2>Оборудование</h2><div class="muted">${esc(group.base)}</div></div><button class="btn tiny" onclick="closeModal()">✕</button></div>${planCard}<div class="field"><label for="mw304Name">Тренажёр или снаряд</label><input id="mw304Name" value="${esc(picked?.label||picked?.model||'')}" placeholder="${cardio?'Например, Assault AirBike Elite':'Например, Matrix Leg Extension'}" autocomplete="off" autocapitalize="words"></div>${history}<button class="btn primary full" onclick="machineWeightSave304('${encodeURIComponent(key)}')">Сохранить</button>`);
      setTimeout(()=>D.getElementById('mw304Name')?.focus(),0)
    }
    function savePicker(token){
      const key=decodeURIComponent(token||''),group=groupFor(key);if(!group)return;
      const label=String(D.getElementById('mw304Name')?.value||'').trim();
      if(!label){W.toast?.('Введи название тренажёра или снаряда');return}
      const id=`custom:${A.slug(label)}`,previous=selected(group)||recentStore()[key],oldProfile=previous&&profileFor(key,previous.id);
      if(previous&&oldProfile&&previous.id!==id&&A.slug(previous.label||previous.model)===A.slug(label)){
        profileStore()[key]??={};profileStore()[key][id]={...oldProfile,id,label,model:label,brand:''}
      }
      const picked={id,label,brand:'',model:label,cardio:isCardio(group),selectedAt:Date.now()};
    ...
    function capture(){
      ...
      groups(cur).forEach(group=>{
        const active=selected(group);if(!active||isCardio(group))return;
    ...
    function control(group){
      const cardio=isCardio(group),active=selected(group),profile=active&&profileFor(group.key,active.id),rec=recommendation(group,profile),count=profile?.samples?.length||0;
      if(!active)return `<button class="mw304-control empty" type="button" onclick="machineWeightOpen304('${encodeURIComponent(group.key)}')"><span><b>Оборудование</b><small>${cardio?'Ввести модель кардиотренажёра':'Ввести тренажёр или снаряд'}</small></span><i>›</i></button>`;
      return `<div class="mw304-control"><button type="button" onclick="machineWeightOpen304('${encodeURIComponent(group.key)}')"><span><b>${esc(active.label)}</b><small>${cardio?'модель сохранена':count?`${count} калибр. · ${rec?`рекомендация ${esc(A.formatWeights(rec.weights))} кг`:'история сохранена'}`:'нужна первая калибровка'}</small></span><i>›</i></button>${rec?`<button class="mw304-apply" type="button" onclick="machineWeightApply304('${encodeURIComponent(group.key)}')">Применить</button>`:''}</div>`
    }
    function enhance(){
      ...
      cards.forEach((card,i)=>{const group=list[i];if(!group)return;const signature=JSON.stringify([group.key,isCardio(group),selected(group)?.id,profileFor(group.key,selected(group)?.id)?.samples?.length||0,recommendation(group,profileFor(group.key,selected(group)?.id))?.weights||[]]);let holder=card.querySelector(':scope > .mw304-holder');if(holder?.dataset.sig===signature)return;if(!holder){holder=D.createElement('div');holder.className='mw304-holder';const row=card.querySelector(':scope > .row');if(row)row.insertAdjacentElement('afterend',holder);else card.prepend(holder)}holder.dataset.sig=signature;holder.innerHTML=control(group)})
    }
  ...
  const isCardioName=name=>/аэро\s*байк|эйр\s*байк|air\s*(?:dyne|bike)|assault\s*bike|echo\s*bike|кардио|велотрен|гребл|эллипс|бегов.*дорож|лыжн.*эрг|ski\s*erg|степпер/.test(String(name||'').toLowerCase());
  const catalog=[
