...
(function(root,factory){
  ...
  function boot(W,A){
    ...
    function pickerChanged(){const id=D.getElementById('mw304Model')?.value||'custom',custom=D.getElementById('mw304Custom');if(custom)custom.classList.toggle('hidden',id!=='custom');const note=D.getElementById('mw304Tech');if(note)note.innerHTML=techNote(id)}
    function openPicker(token){
      const key=decodeURIComponent(token||''),group=groupFor(key);if(!group)return;
      const active=selected(group),recent=recentStore()[key],currentId=active?.id||recent?.id||modelOptions(group)[0]?.id||'barbell';
