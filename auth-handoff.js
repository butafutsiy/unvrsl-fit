...
function authHandoffFinishLogin(user,reason){
 cloud.user=user||null;cloud.profile=null;if(typeof cloudMarkHealthy==='function')cloudMarkHealthy();authHandoffClear();closeModal();renderCloudAffordances();try{render()}catch(_){ }toast('Вход выполнен');if(cloud.user&&typeof cloudDeferProfile==='function')cloudDeferProfile(reason||'handoff');authOfferPasswordSetup();return !!cloud.user
}
async function authHandoffAdoptSession(){if(!cloud?.client)return false;const {data,error}=await cloud.client.auth.getSession();if(error){if(typeof cloudMarkDegraded==='function')cloudMarkDegraded(error);return false}if(data?.session?.user)return authHandoffFinishLogin(data.session.user,'handoff-session');return false}
async function authHandoffPollOnce(showErrors=false){
 ...
 try{
  ...
  if(error){if(typeof cloudMarkDegraded==='function')cloudMarkDegraded(error);if(showErrors)toast('Пока не подтверждено');return false}
  if(data?.token_hash){
   const r=await cloud.client.auth.verifyOtp({token_hash:data.token_hash,type:'email'});
   if(r.error){if(typeof cloudMarkDegraded==='function')cloudMarkDegraded(r.error);if(showErrors)modal(`<div class="sheet-grabber"></div><h2>Не удалось перенести вход</h2><div class="muted">Ссылка была подтверждена, но сессия не перенеслась. Запроси одну новую ссылку и попробуй ещё раз.</div><button class="btn full" onclick="cloudAccountSheet()">Назад</button>`);authHandoffClear();return false}
   return authHandoffFinishLogin(r.data?.session?.user||r.data?.user||null,'handoff-otp')
  }
 ...
 }catch(e){if(typeof cloudMarkDegraded==='function')cloudMarkDegraded(e);if(showErrors)toast('Нет связи с сервером')}finally{authHandoffBusy=false}
 return false
}
...
async function authHandoffPublish(){
 ...
 try{
  ...
  if(error||!data?.ok){if(error&&typeof cloudMarkDegraded==='function')cloudMarkDegraded(error);return false}
 ...
 }catch(e){if(typeof cloudMarkDegraded==='function')cloudMarkDegraded(e);return false}
}
...
window.cloudSendMagic=async function(){
 ...
 try{
  ...
  if(error){if(typeof cloudMarkDegraded==='function'&&(error?.status>=500||String(error?.message||'').toLowerCase().includes('jwt')))cloudMarkDegraded(error);authHandoffClear();if(typeof authFriendlyError==='function'&&authFriendlyError(error,email))return;return modal(`<div class="sheet-grabber"></div><h2>Не удалось отправить письмо</h2><div class="muted">${esc(error.message||'Попробуй ещё раз позже.')}</div><button class="btn full" style="margin-top:18px" onclick="cloudAccountSheet()">Назад</button>`)}
 ...
 }catch(e){if(typeof cloudMarkDegraded==='function')cloudMarkDegraded(e);authHandoffClear();modal(`<div class="sheet-grabber"></div><h2>Ошибка соединения</h2><div class="muted">Не удалось связаться с сервером. Проверь интернет и попробуй позже.</div><button class="btn full" style="margin-top:18px" onclick="cloudAccountSheet()">Назад</button>`)}
};
