...
async function authHandoffAdoptSession(){if(!cloud?.client)return false;const {data}=await cloud.client.auth.getSession();if(data?.session?.user){cloud.user=data.session.user;if(!cloud.profile)await cloudEnsureProfile();authHandoffClear();closeModal();renderCloudAffordances();render();toast('Вход выполнен');return true}return false}
async function authHandoffPollOnce(showErrors=false){
 ...
 try{
  ...
  if(data?.token_hash){
   ...
   cloud.user=r.data?.session?.user||r.data?.user||null;if(cloud.user)await cloudEnsureProfile();authHandoffClear();closeModal();renderCloudAffordances();render();toast('Вход выполнен');return true
  }
...
async function authHandoffPublish(){
 ...
 try{
  ...
  modal(`<div class="sheet-grabber"></div><h2>Вход подтверждён</h2><div class="muted">Готово. Вернись в то окно UNVRSL FIT, где ты запросил ссылку — оно войдёт в аккаунт автоматически.</div><button class="btn primary full" style="margin-top:18px" onclick="closeModal()">Готово</button>`);return true
...
window.cloudSendMagic=async function(){
 ...
 try{
  ...
  modal(`<div class="sheet-grabber"></div><h2>Проверь почту</h2><div class="muted">Открой самое новое письмо и нажми ссылку. Даже если она откроется в Safari, это окно UNVRSL FIT само подхватит вход.</div><div class="card" style="margin-top:16px"><b>${esc(email)}</b><div class="muted small" style="margin-top:5px">Окно можно оставить открытым. После подтверждения вход появится здесь автоматически.</div></div><button class="btn primary full" onclick="authHandoffManualCheck()">Проверить вход</button><button class="btn full" style="margin-top:10px" onclick="cloudAccountSheet()">Назад</button>`)
