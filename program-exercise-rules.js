...
(()=>{
  ...
  window.__unvrslProgramParameterOverridesV381=true;
  ...
  function legacyMode(e,key,isNew){
    ...
    if(key==='reps'&&e?.repMode)return e.repMode;
  ...
  function resolvedParameters(pid,wi,e={}){
    const kind=e.kind||inferKind(e.n||''),defaults=weeklyDefaults(pid,wi,kind),method=String(e.method||'STANDARD').toUpperCase(),overrides=e.parameterOverrides||{};
    const repMode=method==='UNVRSL'||method==='SLDR'?'method':legacyMode(e,'reps',false),repManual=orderedPair(manualValue(e,'reps','min',e.repMin),manualValue(e,'reps','max',e.repMax),defaults.reps),reps=repMode==='manual'?repManual:defaults.reps;
  ...
  window.saveProgramExercise=function(pid,wi,di,nameToken,sourceToken,bpToken,tgToken,eqToken,existingIndex){
    ...
    const previous=old?.parameterOverrides||{},repManual=manualPair('pmReps',orderedPair(previous.reps?.min??old?.repMin,previous.reps?.max??old?.repMax,defaults.reps),1,50),rpeManual=manualPair('pmRpe',orderedPair(previous.effort?.rpeMin??old?.rpeMin??old?.rpe,previous.effort?.rpeMax??old?.rpeMax??old?.rpe,defaults.rpe),1,10),rirManual=manualPair('pmRir',orderedPair(previous.effort?.rirMin??old?.rirMin,previous.effort?.rirMax??old?.rirMax,defaults.rir),0,10),restManual=manualPair('pmRest',orderedPair(previous.rest?.min??old?.restMin??old?.rest,previous.rest?.max??old?.restMax??old?.rest,defaults.rest),0,600);
    ...
    const data={tempo,tempoLight,heavyReps,lightReps,lightWeight,middleSets,middleReps,middleWeight,sldrSets,sldrDrop},sets=buildSets(method,count,w,r,rest,data),parameterOverrides={version:381,reps:{mode:repsMode,min:repManual[0],max:repManual[1]},effort:{mode:effortMode,type:effortType,rpeMin:rpeManual[0],rpeMax:rpeManual[1],rirMin:rirManual[0],rirMax:rirManual[1]},tempo:{mode:tempoMode,value:tempoManual},rest:{mode:restMode,min:restManual[0],max:restManual[1]},weight:{mode:weightMode,value:manualWeight}};
    if(method==='DS'){const start=reps[1],end=reps[0],steps=Array.from({length:sets.length},(_,i)=>Math.round(start+(end-start)*(i/Math.max(1,sets.length-1))));sets.forEach((set,i)=>{set.r=steps[i];set.rMin=reps[0];set.rMax=reps[1];set.targetRepMin=steps[i];set.targetRepMax=steps[i];set.targetRepLabel=String(steps[i])})}else if(method!=='UNVRSL'&&method!=='SLDR')sets.forEach(set=>{set.r=reps[0];set.rMin=reps[0];set.rMax=reps[1];set.targetRepMin=reps[0];set.targetRepMax=reps[1];set.targetRepLabel=rangeLabel(...reps)});
    const obj={...(old||{}),id:old?.id||uid('pex'),n,sourceId:decodeURIComponent(sourceToken||'')||null,bp:decodeURIComponent(bpToken||''),tg:decodeURIComponent(tgToken||''),eq:decodeURIComponent(eqToken||''),kind,method,repMode:repsMode,effortSourceMode:effortMode,effortType,rpe:rpeTarget,tempoMode,tempo,tempoLight:method==='UNVRSL'?tempoLight:null,restMode,rest,weightMode,parameterOverrides,innerRest:INNER_REST[method],heavyReps:method==='UNVRSL'?heavyReps:null,lightReps:method==='UNVRSL'?lightReps:null,lightWeight:method==='UNVRSL'?lightWeight:null,middleSets:method==='UNVRSL'?middleSets:null,middleReps:method==='UNVRSL'?middleReps:null,middleWeight:method==='UNVRSL'?middleWeight:null,miniSets:method==='SLDR'?sldrSets:null,repDrop:method==='SLDR'?sldrDrop:null,note,sets};
    if(repsMode==='manual'){obj.repMin=reps[0];obj.repMax=reps[1];obj.repRange=rangeLabel(...reps)}else{delete obj.repMin;delete obj.repMax;delete obj.repRange}
