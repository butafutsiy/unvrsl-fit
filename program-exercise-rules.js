...
(()=>{
  ...
  let editingLoadExercise={};
  function roundProgramLoad(v){return WorkoutDomain.roundWeight(v,WorkoutDomain.profile(editingLoadExercise,workoutRegistry,st.exerciseWeightProfiles||{}))}
  ...
  function resolvedParameters(pid,wi,e={}){
    if(!e.reps&&window.programModel)e=programModel.normalizeExercise(JSON.parse(JSON.stringify(e)));
  ...
  window.programExerciseForm=function(x){editingLoadExercise=x;
    ...
    modal(`<div class="px-exercise-head"><button class="px-icon-btn" onclick="openProgramEditor('${x.pid}',${x.wi},${x.di})">←</button><div class="px-head-copy grow"><h2>${esc(x.n)}</h2><div class="muted">Настройка упражнения</div></div></div>${window.equipmentEditorField405?.(editingLoadExercise)||''}
  ...
  window.saveProgramExercise=function(pid,wi,di,nameToken,sourceToken,bpToken,tgToken,eqToken,existingIndex){
    ...
    const equipmentProfileId=document.getElementById('pmEquipmentProfile')?.value||null,equipmentProfile=equipmentProfileId?st.equipmentProfiles?.[equipmentProfileId]||null:null;editingLoadExercise={n,sourceId:decodeURIComponent(sourceToken||''),eq:decodeURIComponent(eqToken||''),equipmentProfile};const loadType=WorkoutDomain.loadType(editingLoadExercise,workoutRegistry),rawManual=remembered('pmWeight',previous.weight?.value??old?.sets?.[0]?.w),manualWeight=rawManual??(['bodyweight_only','bodyweight_added','repetitions_only'].includes(loadType)?0:null);if(weightMode==='manual'&&(manualWeight===null||manualWeight<0)){try{toast('Укажи вес или выбери «Авто»')}catch(_){}return}const w=weightMode==='manual'?manualWeight:0;
    ...
    const obj={...(old||{}),id:old?.id||uid('pex'),exerciseId:workoutRegistry.identity(editingLoadExercise),loadType,n,sourceId:decodeURIComponent(sourceToken||'')||null,bp:decodeURIComponent(bpToken||''),tg:decodeURIComponent(tgToken||''),eq:decodeURIComponent(eqToken||''),equipmentProfileId,equipmentProfile:equipmentProfile?{...equipmentProfile}:null,kind,method,reps:canonicalReps,effortSourceMode:effortMode,effortType,rpe:rpeTarget,tempoMode,tempo,tempoLight:method==='UNVRSL'?tempoLight:null,restMode,rest,weightMode,parameterOverrides,innerRest:INNER_REST[method],heavyReps:method==='UNVRSL'?heavyReps:null,lightReps:method==='UNVRSL'?lightReps:null,lightWeight:method==='UNVRSL'?lightWeight:null,middleSets:method==='UNVRSL'?middleSets:null,middleReps:method==='UNVRSL'?middleReps:null,middleWeight:method==='UNVRSL'?middleWeight:null,miniSets:method==='SLDR'?sldrSets:null,repDrop:method==='SLDR'?sldrDrop:null,note,sets};
