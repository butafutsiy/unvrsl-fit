...
test('v256 retirement guard loads before every application module',()=>{
  const html=read('index.html');
  const first=html.match(/<script src="([^"]+)"/);
  assert.equal(first?.[1],'legacy-retirement-v256.js');
});
...
test('active loaders refuse retired scripts and use only canonical Statistics modules',()=>{
  ...
  for(const source of [loader,settings,client])assert.match(source,/unvrslScriptRetiredV256/);
...
test('retired registry blocks every superseded renderer family',()=>{
  const source=read('legacy-retirement-v256.js');
  const appended=[];
  const document={
  ...
  for(const name of ['stats-authority-v252.js','stats-authority-v253.js','stats-dashboard-v2.js','home-stats-v2.js','stats-cleanup.js','og-style-legacy-v157.js','trainer-client-detail-v2.js','client-plan-profile-first-v198.js']){
    assert.equal(window.unvrslScriptRetiredV256(name),true,name);
  }
  assert.equal(window.unvrslScriptRetiredV256('legacy-retirement-v255.js'),true);
  assert.equal(window.unvrslScriptRetiredV256('stats-authority-v254.js'),false);
  assert.equal(window.unvrslScriptRetiredV256('stats-dashboard-v254.js'),false);
  ...
  assert.equal(appended[0]?.id,'legacy-retirement-v256-style');
});
...
test('superseded source files are removed from the published tree',()=>{
  ...
  const active=fs.readdirSync(root).filter(name=>name.endsWith('.js')&&name!=='legacy-retirement-v256.js');
  for(const name of retired){
...
test('client Home and Plan have one canonical owner',()=>{
  ...
  assert.match(runtime,/await script\('client-program-picker\.js\?v=256'\)[\s\S]*await script\('client-journal-profile-v107\.js\?v=256'\)/);
