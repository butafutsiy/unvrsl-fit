...
test('v395 uses one cache version across static and dynamic loaders',()=>{
 for(const name of ['index.html','frequent-patch.js']){
  const versions=[...read(name).matchAll(/\?v=(\d+)\b/g)].map(x=>x[1]);
  assert.ok(versions.length);assert.deepEqual([...new Set(versions)],['395']);
 }
 assert.match(read('startup-orchestrator.js'),/RELEASE=395/);
 assert.match(read('sw.js'),/SW_RELEASE = "v395"/);
