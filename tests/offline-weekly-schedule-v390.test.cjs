...
test('runtime loads one canonical clients owner before the cloud chain settles',()=>{
  ...
  assert.match(loader,/trainer-direct-ui\.js'[\s\S]*offline-progress\.js\?v=411[\s\S]*offline-schedule\.js\?v=411[\s\S]*trainer-clients-canonical\.js\?v=411/);
...
test('startup keeps the trainer shell hidden until the canonical owner is ready',()=>{
  ...
  assert.match(index,/window\.__unvrslRelease='v411'/);
  assert.match(index,/frequent-patch\.js\?v=411/);
  assert.match(sw,/v411/)
});
