...
test('runtime loads one canonical clients owner before the cloud chain settles',()=>{
  ...
  assert.match(loader,/trainer-direct-ui\.js'[\s\S]*offline-progress\.js\?v=406[\s\S]*offline-schedule\.js\?v=406[\s\S]*trainer-clients-canonical\.js\?v=406/);
...
test('startup keeps the trainer shell hidden until the canonical owner is ready',()=>{
  ...
  assert.match(index,/window\.__unvrslRelease='v406'/);
  assert.match(index,/frequent-patch\.js\?v=406/);
  assert.match(sw,/v406/)
});
