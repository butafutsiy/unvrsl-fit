...
test('iOS uses a PNG touch icon and a versioned service worker',()=>{
  ...
  assert.match(html,/apple-touch-icon[^>]+apple-touch-icon\.png\?v=428/);
  assert.match(html,/serviceWorker\.register\('\.\/sw\.js\?v=428'/);
...
test('service worker isolates current shell and media from user data',()=>{
 const sw=read('sw.js');
 assert.match(sw,/SW_RELEASE = "v428"/);
...
test('every local script and stylesheet linked from the shell exists',()=>{
 ...
 assert.match(html,/equipment-profiles-v405\.js\?v=428/);
});
