...
test("23: activate removes old app caches while preserving unrelated caches", async () => {
  ...
  assert.ok(r.buckets.has("unvrsl-shell-v395"));
});
test("offline launch uses the fully installed current HTML and versioned scripts", async () => {
  ...
  assert.match(await html.text(), /workout-store.js\?v=395/);
  const script = await r.event("fetch", {
    request: { url: "https://app.test/unvrsl-fit/app.js?v=395", method: "GET" },
  });
  assert.match(await script.text(), /app.js\?v=395/);
  const owner = await r.event('fetch',{request:{url:'https://app.test/unvrsl-fit/trainer-shell.js?v=395',method:'GET'}});
  assert.match(await owner.text(),/trainer-shell.js\?v=395/);
});
test("a v391 URL can never satisfy a v395 script request", async () => {
  const r = runtime(),
    cache = await r.caches.open("unvrsl-shell-v395");
