...
test("23: activate removes old app caches while preserving unrelated caches", async () => {
  ...
  assert.ok(r.buckets.has("unvrsl-shell-v418"));
});
test("offline launch uses the fully installed current HTML and versioned scripts", async () => {
  ...
  assert.match(await html.text(), /workout-store.js\?v=418/);
  const script = await r.event("fetch", {
    request: { url: "https://app.test/unvrsl-fit/app.js?v=418", method: "GET" },
  });
  assert.match(await script.text(), /app.js\?v=418/);
  const owner = await r.event('fetch',{request:{url:'https://app.test/unvrsl-fit/trainer-shell.js?v=418',method:'GET'}});
  assert.match(await owner.text(),/trainer-shell.js\?v=418/);
});
test("a v391 URL can never satisfy a v418 script request", async () => {
  const r = runtime(),
    cache = await r.caches.open("unvrsl-shell-v418");
