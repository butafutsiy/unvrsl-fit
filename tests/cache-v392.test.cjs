...
test("23: activate removes old app caches while preserving unrelated caches", async () => {
  ...
  assert.ok(r.buckets.has("unvrsl-shell-v403"));
});
test("offline launch uses the fully installed current HTML and versioned scripts", async () => {
  ...
  assert.match(await html.text(), /workout-store.js\?v=403/);
  const script = await r.event("fetch", {
    request: { url: "https://app.test/unvrsl-fit/app.js?v=403", method: "GET" },
  });
  assert.match(await script.text(), /app.js\?v=403/);
  const owner = await r.event('fetch',{request:{url:'https://app.test/unvrsl-fit/trainer-shell.js?v=403',method:'GET'}});
  assert.match(await owner.text(),/trainer-shell.js\?v=403/);
});
test("a v391 URL can never satisfy a v403 script request", async () => {
  const r = runtime(),
    cache = await r.caches.open("unvrsl-shell-v403");
