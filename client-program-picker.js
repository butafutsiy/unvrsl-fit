...
(()=>{
  ...
  function clientPlanPageV3(){
    ...
    const card=x=>`<button class="client-program-choice ${x.key===p.key?'on':''}" onclick="clientPlanSelectProgram('${encodeURIComponent(x.key)}')">${x.key===primary?.key?'<span class="client-program-badges"><span class="client-program-badge primary">Основная</span></span>':''}<b>${escx(x.name)}</b><span>${weekCount(x)} нед.</span></button>`;
    const programGroups=ps.length>1?`<div class="client-program-group"><div class="section">МОИ ПРОГРАММЫ</div><div class="client-program-strip">${ps.map(card).join('')}</div></div>`:'';
