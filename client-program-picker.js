...
(()=>{
  ...
  function allClientPrograms(){
    ...
    (Array.isArray(st.programs)?st.programs:[]).forEach(p=>{
      if(!p||p.archived||!p.cloudPlanId||!p.trainerId||!Array.isArray(p.weeks)||!p.weeks.length)return;
    ...
    (Array.isArray(st.remotePlans)?st.remotePlans:[]).forEach(p=>{
      const rs=p?.snapshot?.routines;if(!p||!p.trainerId||!Array.isArray(rs)||!rs.length)return;
  ...
  function ensurePrimary(){
    const ps=allClientPrograms();if(!ps.length){delete st.clientPrimaryProgramKey;return null}
  ...
  function setSavedWeek(p,n){if(!st.clientProgramWeeks||typeof st.clientProgramWeeks!=='object')st.clientProgramWeeks={};st.clientProgramWeeks[p.key]=n;try{save()}catch(e){}}

  let picker={key:null,week:null};
  function renderStartPicker(){
    const ps=orderedPrograms();
    if(!ps.length){modal('<div class="sheet-grabber"></div><div class="row between"><h2>Выбрать тренировку</h2><button class="btn tiny" onclick="closeModal()">✕</button></div><div class="card muted">Тренер пока не назначил программу.</div>');return}
    ...
    sh?.querySelectorAll('[data-client-start-day]').forEach(btn=>btn.onclick=()=>{const d=weekDays(p,w-1)[Number(btn.dataset.clientStartDay)];if(!d)return;st.clientLastProgramKey=p.key;setSavedWeek(p,w);try{save()}catch(e){};d.start()});
  }
  ...
  function clientPlanPageV3(){
    ...
    if(!window.cloud?.user){root.innerHTML='<div class="card"><div class="title">Мой план</div><div class="muted" style="margin-top:7px">Войди, чтобы получить программу от тренера.</div><button class="btn primary full" style="margin-top:14px" onclick="cloudAccountSheet()">Войти</button></div>';return}
    const ps=orderedPrograms();if(!ps.length){root.innerHTML='<div class="card"><div class="title">План пока не назначен</div><div class="muted" style="margin-top:7px">Когда тренер назначит программу, она появится здесь.</div></div>';return}
  ...
  if(typeof oldAssignments==='function'&&!oldAssignments.__clientPicker){
    const wrapped=async function(){const r=await oldAssignments.apply(this,arguments);ensurePrimary();try{save()}catch(e){};return r};
