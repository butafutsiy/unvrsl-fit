...
(()=>{
  ...
  const t=setInterval(wrap,250);setTimeout(()=>{clearInterval(t);setInterval(wrap,1500)},20000);
  document.addEventListener('click',e=>{if(e.target?.closest?.('.nav button[data-p="plan"]')&&isClient())setTimeout(inject,50)},true);
  let queued=false;new MutationObserver(()=>{const root=document.getElementById('plan');if(!isClient()||!root?.classList.contains('active')||root.querySelector('.client-plan-profile-v198')||queued)return;queued=true;setTimeout(()=>{queued=false;inject()},0)}).observe(document.getElementById('plan')||document.body,{childList:true,subtree:true});
