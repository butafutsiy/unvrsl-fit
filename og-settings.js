...
function settingsSheet(){
  ...
  <div class="section">ДАННЫЕ</div><div class="settings-card"><div class="setting"><div><b>Русская база упражнений</b><div class="muted small">${ogLibraryLoaded?ogLibrary.length:'1324'} упражнений · картинки, анимации и техника на русском</div></div><button class="btn tiny" onclick="loadExerciseDB(true);toast('Обновляю базу')">Обновить</button></div><div class="setting"><div><b>Импорт из openGym</b><div class="muted small">Резервная копия</div></div><label class="btn tiny" for="ogImport">Импорт</label><input id="ogImport" type="file" accept=".json,application/json" hidden onchange="importOpenGym(this.files[0])"></div><div class="setting"><div><b>Экспорт для ChatGPT</b><div class="muted small">Тренировки, RPE и вес</div></div><button class="btn tiny" onclick="exportChat()">Экспорт</button></div><div class="setting"><div><b>Экспорт резервной копии</b><div class="muted small">Все локальные данные</div></div><button class="btn tiny" onclick="backup()">Файл</button></div><div class="setting"><div><b>Импорт резервной копии</b></div><label class="btn tiny" for="bkImport">Импорт</label><input id="bkImport" type="file" accept=".json,application/json" hidden onchange="restoreBackup(this.files[0])"></div><div class="setting"><button class="btn danger full" onclick="resetAll()">Сбросить всё</button></div></div>`)
}
...
exercisesPage=function(){
  ...
  const chip=document.querySelector('#exercises .catalog-head .chip');
